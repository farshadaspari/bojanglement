...
@Component({
    selector: 'app-pseudolocalization',
    templateUrl: './pseudolocalization.component.html',
    styleUrls: ['./pseudolocalization.component.css']
})
export class PseudolocalizationComponent implements OnInit {
    ...
    pseudolocalize(value: string): void {
        ...
        lines.forEach((l, i) => {
            let characters = l.split("");
            characters.forEach((c) => {
                this.destinationValue = this.destinationValue + this.pseudoCharacter(c);
            });
            if (i !== l.length - 1) {
